...
La version de développement peut être testée à l'adresse suivante : <https://pims-demo.fr>.

## :japanese_castle: Architecture

Le projet numérique de PIMS dématérialisé est construit sur 2 repository Github :

- [pims-frontend](https://github.com/pims-online/pims-frontend) : Application React pour créer l'interface cliente accessible sur le web, et générer le contenu du PIMS. Réalisée avec le framework Vite.
- [pims-backend](https://github.com/pims-online/pims-backend) : Application Flask pour donner accès à des API complémentaires concernant les risques, et surtout générer le PDF.

Obsolète :
- [pims-proxy](https://github.com/pims-online/pims-proxy) : Des fonctions serverless pour opérer en proxy sur certaines requêtes du client pims-frontend. Ce proxy n'est plus requis au fonctionnement de l'application.
...
- Possibilité de passer l'adresse dans l'url lors de la navigation vers le site, via le query params `address` : <https://pims-demo.fr/?address=60+Boulevard+Saint+Michel+75005+Paris> ; 
